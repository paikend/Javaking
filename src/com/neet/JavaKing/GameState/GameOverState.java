...
public class GameOverState extends GameState {
	...
	public void init() {
		...
		ticks = Data.getTime();
		if(ticks < 6600) rank = 1;
		else if(ticks < 8400) rank = 2;
		else if(ticks < 107200) rank = 3;
		else rank = 4;
	}
	...
	public void draw(Graphics2D g) {
		...
		if(rank == 1) Content.drawString(g, "Java King", 20, 78);
		else if(rank == 2) Content.drawString(g, "Java Knight", 24, 78);
		else if(rank == 3) Content.drawString(g, "Java Human", 32, 78);
		else if(rank == 4) Content.drawString(g, "Java idiot", 8, 78);
	...
	public void handleInput() {
		if(Keys.isPressed(Keys.ENTER)) {
			gsm.setState(GameStateManager.NAME);
