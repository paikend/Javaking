...
import com.neet.JavaKing.TileMap.Tile;
...
public abstract class Entity {
	...
	public boolean validateNextPosition() {
		...
		if(left) {
			if(x<0 ||colTile == 0 || tileMap.getType(rowTile, colTile - 1) == Tile.BLOCKED) {
				return false;
			}
			else {
				xdest = x - tileSize;
		...
		if(right) {
			if(x>1008 || colTile == tileMap.getNumCols() || tileMap.getType(rowTile, colTile + 1) == Tile.BLOCKED) {
				return false;
			}
			else {
				xdest = x + tileSize;
		...
		if(up) {
			if(y<0 || rowTile == 0 || tileMap.getType(rowTile - 1, colTile) == Tile.BLOCKED) {
				return false;
			}
			else {
				ydest = y - tileSize;
		...
		if(down) {
			if(y>1008 || rowTile == tileMap.getNumRows() - 1 || tileMap.getType(rowTile + 1, colTile) == Tile.BLOCKED) {
				return false;
			}
			else {
				ydest = y + tileSize;
